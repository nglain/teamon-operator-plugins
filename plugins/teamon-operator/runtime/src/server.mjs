...
export function createOperatorMcpServer(config, dependencies, {initialState} = {}) {
  ...
  server.registerTool('operator_workspace_open', workspaceDefinition, openWorkspace);
  ...
  if (hasCore) {
    ...
    registerJournalledTool("operator_reminders_read", {
    ...
    registerJournalledTool("operator_provider_auth_start", {
    ...
    registerJournalledTool("agent_change_prepare", {
      description: "Core: prepare one native revision-bound agent adaptation, not apply it. Read agent_configuration_read and relevant document/operator_reminders_read first. Semantic changes: settings, identity patch, context/local skill document, skill/connector assignment, provider continuity or reminder.change. No max_turns/guard switches or raw credentials. Preview exact change/reason before human approval/operation_commit.",
